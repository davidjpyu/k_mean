...
def calculate_centers(
    ...
    center_freq = np.zeros(num_centers)
    
    for i in range(data.shape[0]):
        centers[classifications[i]] += data[i]
        center_freq[classifications[i]] += 1
    
    for i in range(num_centers):
        centers[i] /= center_freq[i]
...
def cluster_data(data: np.ndarray, centers: np.ndarray) -> np.ndarray:
    ...
    for i in range(data.shape[0]):
        classification[i] = np.argmin(np.sum((centers - data[i]) ** 2, axis = 1))
...
def lloyd_algorithm(
    ...
    oldcenters = data[-num_centers:,:]

    err_itr = []
    while np.max(abs(centers - oldcenters)) > epsilon:
        oldcenters = centers
        clusters = cluster_data(data, centers)
        centers = calculate_centers(data, clusters, num_centers)
        err_itr.append(calculate_error(data, centers))
    
    return (centers, err_itr)
